...
export type HlsManifestAuthContext = {
  manifestUrl: string;
  manifestOrigin: string;
  authEntries: readonly (readonly [string, string])[];
};

export type HlsFatalEventData = {
  fatal?: boolean;
  response?: {
    code?: unknown;
  } | null;
...
export interface HlsRuntime {
  ...
  create(config: HlsRuntimeConfig): HlsInstanceLike;
}
...
const defaultHlsRuntime: HlsRuntime = {
  ...
  DefaultConfig: {
    get loader() {
      return Hls.DefaultConfig.loader as HlsLoaderCtor;
    },
  },
  create(config) {
    return new Hls(config as unknown as Partial<HlsConfig>) as unknown as HlsInstanceLike;
  },
};

export const DEFAULT_HLS_AUTH_PARAM_NAMES = [
  "token",
  "bcdn_token",
  "expires",
  "token_path",
...
export function normalizeSeekTarget(seconds: number): number {
...
export function buildManifestAuthContext(
  manifestUrl: string,
  authParamNames: readonly string[] = DEFAULT_HLS_AUTH_PARAM_NAMES,
): HlsManifestAuthContext {
  const parsedManifestUrl = new URL(manifestUrl);

  return {
    manifestUrl,
    manifestOrigin: parsedManifestUrl.origin,
    authEntries: authParamNames
      .map((name) => [name, parsedManifestUrl.searchParams.get(name)] as const)
...
export function appendAuthParams(
  rawUrl: string,
  authContext: HlsManifestAuthContext,
): string {
  if (!authContext.authEntries.length) {
    return rawUrl;
  }

  let resolvedUrl: URL;
  try {
    resolvedUrl = new URL(rawUrl, authContext.manifestUrl);
  } catch {
    return rawUrl;
  }

  if (resolvedUrl.origin !== authContext.manifestOrigin) {
    return resolvedUrl.toString();
  }

  for (const [name, value] of authContext.authEntries) {
...
export function mapHlsFatalEvent(
  data: HlsFatalEventData,
): HlsFatalError | null {
...
function createBunnyTokenLoader(
  BaseLoader: HlsLoaderCtor,
  authContext: HlsManifestAuthContext,
): HlsLoaderCtor {
  return class BunnyTokenLoader extends BaseLoader {
    load(
      context: LoaderContext,
      config: LoaderConfiguration,
      callbacks: LoaderCallbacks<LoaderContext>,
    ): void {
      context.url = appendAuthParams(context.url, authContext);
...
export class HlsPlaybackAdapter {
  ...
  private hls: HlsInstanceLike | null = null;
  ...
  constructor(private readonly runtime: HlsRuntime = defaultHlsRuntime) {}
  ...
  async initialize(video: HTMLVideoElement, manifestUrl: string): Promise<void> {
    ...
    const userAgent =
      typeof navigator !== "undefined" && typeof navigator.userAgent === "string"
        ? navigator.userAgent
        : "";
    const hlsJsSupported = this.runtime.isSupported();
    ...
    const authContext = buildManifestAuthContext(manifestUrl);
    const BunnyTokenLoader = createBunnyTokenLoader(
      this.runtime.DefaultConfig.loader,
      authContext,
    );

    const hls = this.runtime.create({
      enableWorker: true,
    ...
    hls.on(this.runtime.Events.MANIFEST_PARSED, () => {
      if (this.isGenerationStale(generation)) {
        return;
      }
    ...
    hls.on(this.runtime.Events.MEDIA_ATTACHED, () => {
      if (this.isGenerationStale(generation)) {
        return;
      }
      this.mediaAttached = true;
    ...
    hls.on(this.runtime.Events.ERROR, (_event, data) => {
      if (this.isGenerationStale(generation)) {
        return;
      }
      const mappedError = mapHlsFatalEvent((data ?? {}) as HlsFatalEventData);
      if (!mappedError) {
        return;
      }
      this.emitFatal(mappedError);
    });

    hls.on(this.runtime.Events.SUBTITLE_TRACKS_UPDATED, (_event, data) => {
      if (this.isGenerationStale(generation)) {
        return;
      }
      const subtitleTracks = (
        (data as { subtitleTracks?: Array<{ id: number; lang?: string; name?: string }> })
          ?.subtitleTracks ?? []
      );
      const track = subtitleTracks.find(
        (entry) =>
          entry.lang === "en" || entry.name?.toLowerCase().includes("english"),
      );
      if (!track) return;
  ...
  async waitUntilReady(timeoutMs: number): Promise<void> {
    await Promise.race([
      ...
      new Promise<void>((_, reject) => {
        ...
        void this.readyPromise.finally(() => clearTimeout(timer));
      }),
    ]);
  }
